...
function PaymentsManager({ payments, onChange }: { payments: any[]; onChange: () => void }) {
  ...
  const empty = () => ({
    ...
    type: "bank",
    notes: "",
    order_index: payments.length + 1,
    active: true,
  });

  return (
    <div>
      <SectionHead title="Metode Pembayaran" desc="Kelola rekening bank & e-wallet yang tampil di landing page.">
        <button onClick={() => setEditing(empty())} className="inline-flex items-center gap-1 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition hover:opacity-90">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {payments.map((p) => (
          <div key={p.id} className={`rounded-2xl border p-5 shadow-[var(--shadow-card)] ${p.active ? "border-border bg-card" : "border-dashed border-border bg-secondary/40 opacity-70"}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 min-w-0">
                {p.logo_url ? (
                  <img src={p.logo_url} alt={p.bank_name} className="h-10 w-10 rounded-lg object-contain bg-background p-1 ring-1 ring-border" />
                ) : (
                ...
                <div className="min-w-0">
                  <div className="text-xs font-semibold uppercase text-muted-foreground">{p.type === "ewallet" ? "E-Wallet" : "Bank"}</div>
                  <div className="font-bold">{p.bank_name}</div>
                  <div className="font-mono text-sm">{p.account_number}</div>
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => setEditing(p)} className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-medium hover:bg-secondary">Edit</button>
                <button onClick={async () => { if (confirm(`Hapus "${p.bank_name}"?`)) { await del({ data: { id: p.id } }); onChange(); } }} className="inline-flex items-center justify-center gap-1 rounded-lg bg-destructive/10 px-3 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/20"><Trash2 className="h-3.5 w-3.5" /> Hapus</button>
              </div>
            </div>
          </div>
        ))}
        {payments.length === 0 && (
          <div className="col-span-full rounded-2xl border border-dashed border-border bg-card p-8 text-center text-sm text-muted-foreground">
            Belum ada metode pembayaran. Klik "Metode Baru" untuk menambahkan.
          </div>
        )}
      </div>
      {editing && (
        <EditorModal title={editing.id ? "Edit Metode Pembayaran" : "Metode Pembayaran Baru"} onClose={() => setEditing(null)}>
          <PaymentForm initial={editing} onSubmit={async (v) => { await save({ data: v }); setEditing(null); onChange(); }} />
        </EditorModal>
      )}
    </div>
  );
...
function PaymentForm({ initial, onSubmit }: { initial: any; onSubmit: (v: any) => Promise<void> }) {
  const [v, setV] = useState({ ...initial });
  const [saving, setSaving] = useState(false);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); setSaving(true); try { await onSubmit(v); } finally { setSaving(false); } }} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <Field label="Tipe">
      ...
      <Field label="Nama Bank / E-Wallet"><input className={inputCls} value={v.bank_name} onChange={(e) => setV({ ...v, bank_name: e.target.value })} required placeholder="Bank BCA / DANA / OVO" /></Field>
      <Field label="Nomor Rekening / HP"><input className={inputCls} value={v.account_number} onChange={(e) => setV({ ...v, account_number: e.target.value })} required /></Field>
      <Field label="Atas Nama"><input className={inputCls} value={v.account_name} onChange={(e) => setV({ ...v, account_name: e.target.value })} required /></Field>
      <Field label="URL Logo (opsional)"><input className={inputCls} value={v.logo_url} onChange={(e) => setV({ ...v, logo_url: e.target.value })} placeholder="https://..." /></Field>
      <Field label="Catatan (opsional)"><textarea rows={2} className={inputCls} value={v.notes} onChange={(e) => setV({ ...v, notes: e.target.value })} /></Field>
      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={v.active} onChange={(e) => setV({ ...v, active: e.target.checked })} /> Aktif (tampilkan di situs)</label>
      <button disabled={saving} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[image:var(--gradient-hero)] py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-60">
        {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Simpan
      </button>
